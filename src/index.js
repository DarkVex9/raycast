...
function mapXToScreen(value) {
	return value * mapCamera.gridSize - mapCamera.x * mapCamera.gridSize + mapCanvas.width / 2;
}
function mapYToScreen(value) {
	return value * mapCamera.gridSize - mapCamera.y * mapCamera.gridSize + mapCanvas.height / 2;
}
...
function mapDrawCircle(x, y, r, color = "#ccc") {
	...
	mapctx.strokeStyle = color;
	mapctx.beginPath();
	mapctx.arc(mapXToScreen(x), mapYToScreen(y), r * mapCamera.gridSize, 0, 2 * Math.PI);
	mapctx.stroke();
	mapctx.strokeStyle = oldColor;
}
...
function castRay(
	...
	distance = startDistance + Math.sqrt(Math.pow(x - rayX, 2) + Math.pow(y - rayY, 2));
	startX = rayX;
	startY = rayY;
	let lineSteps = [x, y];
	modX = 1 / tanAngle;
	...
	modDistance = (1 / Math.sin(currentAngle)) * Math.cos(referenceAngle - currentAngle);
	//mapctx.strokeStyle = "#F00";
	if (currentAngle % (2 * pi) < pi * 0.5 || currentAngle % (2 * pi) > pi * 1.5) {
		tanAngle = Math.tan(currentAngle); //tan probably should not be defined differently on top and bottom
	...
	if (currentAngle % (2 * pi) > 0.5 * pi && currentAngle % (2 * pi) < 1.5 * pi) {
		tanAngle = Math.tan(pi - currentAngle); //tan probably should not be defined differently on top and bottom
	...
	while (
		...
		*/
		...
		rayX += modX;
		if (iteration === 0) {
			modDistance = Math.sqrt(Math.pow(startX - rayX, 2) + Math.pow(startY - rayY, 2));
		}
		distance += modDistance;
		iteration++;
		//mapDrawCircle(rayX, rayY, 0.04, "#80f");
...
function render() {
	...
	let currentAngle = (((camera.position.angle - camera.fov / 2) % (2 * pi)) + 2 * pi) % (2 * pi);
	...
	for (let rayIndex = 0; rayIndex < camera.rays; rayIndex++) {
      ...
      console.log("new: " + distance + "\n");
		...
		for (let i = 0; i < hit.length; i++) {
			let columnHeight =
				(wallSize / hit[i].opticalDistance) * camera.focalDistance * camera.pxPerRay;
			if (
				...
				drawTexture(
					...
					Math.max(
						Math.min(
							1 / (hit[i].distance / camera.light - hit[i].NS * camera.fakeLight),
							1
						),
						0.1
					)
...
function initializeComponents() {
	Object.keys(component).forEach(function (key, index) {
		...
		if (component[key].hasOwnProperty("update") && updatingComponents.indexOf(key) === -1) {
...
component.velocity.description = "stores x and y velocity, update changes position acordingly";
...
component.gravity = {
	...
	add: function (object, strength = 1, variable = ["velocity"]["y"], doUpdate = true) {
		object.gravity = {};
	...
	update: function () {
		for (let i = 0; i < this.updateList.length; i++) {
			let object = ent[this.updateList[i]];
			object[object.gravity.variable[0]][object.gravity.variable[1]] += object.gravity.force;
		}
	},
